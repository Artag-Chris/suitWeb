...
export default function WebinarMicroserviciosPage() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentMetric((prev) => (prev + 1) % benefits.length)
    }, 4000)
          ...
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              {benefits.map((benefit, index) => (
                <div
                  key={index}
                  className={`group relative bg-gradient-to-br from-slate-900/80 to-slate-800/80 backdrop-blur-sm border rounded-2xl p-6 transition-all duration-500 hover:transform hover:scale-105 ${
                    currentMetric === index
